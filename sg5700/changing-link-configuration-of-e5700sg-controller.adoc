...
xref:placing-appliance-into-maintenance-mode.adoc[E5700SG コントローラをメンテナンスモードにしてください]。
...
+
VLAN 設定を変更した場合は、アプライアンスのサブネットが変更されている可能性があります。アプライアンスの IP アドレスを変更する必要がある場合は、に従います xref:setting-ip-configuration-sg5700.adoc[IP を設定します] 手順

. StorageGRID アプライアンスインストーラから、 *Configure Networking*>*Ping Test* を選択します。
. ping テストツールを使用して、で行ったリンク設定の変更によって影響を受けた可能性のあるネットワーク上の IP アドレスへの接続を確認します ,リンク設定を変更します ステップ。
+
